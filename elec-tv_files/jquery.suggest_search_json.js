(function($) {
	$.suggest = function(input, options) {
		...
		$input.on('change focus keyup',function(e){
		...
		$input.blur(function(event) {
			...
			setTimeout(function(event) { 
				//
				//if(target.getAttribute('id')!="srhkw1"){
...
*/
		...
		function suggest_nokw() {
			...
			$.get(options.source, {q: "getAD"}, function(res) {


				var kk=["apple","asus","acer","手機","筆電","電暖器","機車","電腦","玩具"];
				for(i=0;i<kk.length;i++){
					res.kws[i] = { "kw": kk[i], "child": false };
      ...
      q = ww[Math.floor(Math.random()*ww.length)];
			...
			if (q.length >= options.minchars) {
				...
				if (false) {
					...
					if(!notchange){
						...
						$.get(options.source, {q: q}, function(res) {
		...
		function show_sub_kw(){
			...
			top_ul.show().css('padding-top', top_ul.top).prev('a').addClass('active');
		...
		function displayItems(items,j) {
			...
			var html = '<ul id="dropdown">';
                ...
                for (var j = 0; j < items.ads.length; j++){
                	html += "<li class='ez_ad_border'><a class='suggestion ez_ad' href='"+items.ads[j].url+"' target='_blank' onmousedown=\"_gaq.push([\'_trackEvent\', \'suggestAd\', \'Ad\',\'"+items.ads[j].mid+"\']);\"><span class='suggest'>"+items.ads[j].title+"</span></a></li>";
                }

				for (var i = 0; i < items.kws.length; i++){
					//var tmpidr = items[i].split("fa-arrow-position'");
					var tmpid = '';
					if(items.kws[i].child){
						
						sub_kw_list(items.kws[i].kw,i);
					    sub_kw = "<ul class='sub_kw_list'><h3>"+items.kws[i].kw+"</h3><li class='tag_group'><ul class='clearfix' id='sub_kw_"+i+"'></ul></li></ul>";
						
						html += '<li><a href="http://ezprice.com.tw/price_list.php?kw=' + items.kws[i].kw + '" class="suggestion"  onmousedown="_gaq.push([\'_trackEvent\', \'suggestKw\', \'Kw\',\''+items.kws[i].kw+'\']);"><span class="suggest">' + items.kws[i].kw + '<i class="fa fa-angle-right fa-lg fa-arrow-position"></i></span><div class="clear"></div></a>' + sub_kw + '</li>';
					
					}else{

						if(tmpid.match(/\/ez-skw-level-3\//g)){
							tmpid = $.trim(tmpid.replace(/\/ez-skw-level-3\//g,""));
							html += '<li><a href="http://ezprice.com.tw/price_list.php?kw=' + items.kws[i].kw + '" class="suggestion"  onmousedown="_gaq.push([\'_trackEvent\', \'suggestKw\', \'sKw\',\''+items.kws[i].kw+'\']);"><span class="suggest">' + items.kws[i].kw + '</span><div class="clear"></div></a></li>';
						}else{
							html += '<li><a href="http://ezprice.com.tw/price_list.php?kw=' + items.kws[i].kw + '" class="suggestion"  onmousedown="_gaq.push([\'_trackEvent\', \'suggestKw\', \'Kw\',\''+items.kws[i].kw+'\']);"><span class="suggest">' + items.kws[i].kw + '</span><div class="clear"></div></a></li>';
						}
